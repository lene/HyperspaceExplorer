...
#   endif
    ...
    std::vector<VecMath::Vector<D> > Xold = VertexData<D>::X();
    for (unsigned i = 1; i <= _numSegments; ++i) {
        ...
        for (typename std::vector<VecMath::Vector<D> >::iterator j = Xold.begin();
             j != Xold.end(); ++j) {
                 VertexData<D>::X().push_back(rotate_vertex(*j, rot, d, d-1));
        }
    }
    ...
    new_realm.setAssociatedVertices(VertexData<D>::vertices());
...
#   endif
    ...
    VertexData<D>::X().pop_back();
    for (unsigned i = 1; i <= _numSegments; ++i) {
        double rot = 2.*i*M_PI/_numSegments;
        VecMath::Vector<D> current = VertexData<D>::X()[0];
        VecMath::Vector<D> rotated_current = rotate_vertex(current, rot,
                                                            d, d-1);
        VertexData<D>::X().push_back(rotated_current);
        new_surface.push_back(VertexData<D>::X().size()-1);
    }
...
#   endif
    ...
    for (unsigned i = 1; i <= _numSegments; ++i) {
        ...
        unsigned cur_index = current_surface[0],
            ...
            VecMath::Vector<D> current = VertexData<D>::X()[cur_index];
            VecMath::Vector<D> next = VertexData<D>::X()[next_index];
            VecMath::Vector<D> rotated_current = rotate_vertex(current, rot,
                                                               d, d-2);
            ...
            VertexData<D>::addSurface(old_current, next_index,
                                       VertexData<D>::X().size()-1);

            old_current = VertexData<D>::X().size()-1;
    }
...
#   endif
    ...
    VertexData<D>::X().push_back(VertexData<D>::X()[current_surface[0]]);
    ...
    for (unsigned i = 1; i <= (_numSegments+1)/2; ++i) {
        ...
        for (unsigned j = 0; j < current_surface.size(); j++) {
            ...
            VecMath::Vector<D> current = VertexData<D>::X()[cur_index-offset];
            VecMath::Vector<D> next = VertexData<D>::X()[next_index-offset];
            VecMath::Vector<D> rotated_current = rotate_vertex(current, rot,
                                                               d, d-1);
            VecMath::Vector<D> rotated_next = rotate_vertex(next, rot, d, d-1);
